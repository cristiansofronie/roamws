                 ...
                 void *user, void *in, size_t len)
{
        ...
        case LWS_CALLBACK_CLIENT_ESTABLISHED:
        {
            ...
            if (pid == -1) {
              ...
              execl("/usr/bin/dmenu", "dmenu", "-f", NULL);
            ...
            int capacity = BUFSIZ;
            int size = 0;
            while (read_n = read(filedes[0], buffer + size, 4096)) {
              size += read_n;
              if (size >= capacity) {
                capacity *= 2;
            ...
            unsigned char buf[LWS_SEND_BUFFER_PRE_PADDING + strlen(json_message_before) + size + strlen(json_message_after) + LWS_SEND_BUFFER_POST_PADDING];
