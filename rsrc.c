                            ...
                            size_t len)
{
    ...
    case LWS_CALLBACK_CLIENT_ESTABLISHED: {
        ...
        size_t read_n = read(filedes[0], buffer, BUFSIZ);
        if (read_n < 2) {
            exit(1);
        }
        ...
        for (int i = 0; i < read_n; i++) {
            if (buffer[i] == '\\') {
                new_buffer[new_buf_i++] = '\\';
                new_buffer[new_buf_i++] = buffer[i];
            } else {
                new_buffer[new_buf_i++] = buffer[i];
            }
        }
        ...
        unsigned char buf[LWS_SEND_BUFFER_PRE_PADDING + strlen(json_message_before) + read_n +
                          strlen(json_message_after) + LWS_SEND_BUFFER_POST_PADDING];
        unsigned char *p = &buf[LWS_SEND_BUFFER_PRE_PADDING];
        size_t n = sprintf((char *)p, "%s%s%s", json_message_before, new_buffer, json_message_after);
